...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
      ...
      int numvert = (vert-1)*horiz;
      int numhor = vert*(horiz-1);
      int row,col;
      
      DisjointSets S = new DisjointSets(horiz*vert);// = set of all cells seperated;

      
      while ( numvert >0 || numhor >0 ){
          if ( random.nextInt(10) %2 == 0 ){// add vert for even
              row = random.nextInt(vert);
              col = random.nextInt(horiz-1);
              if(vWalls[col][row]){
                  int cell1=row*horiz+col;
                  int cell2=cell1+1;
                  
                  if( S.find(cell1) != S.find(cell2)){
                      S.union(S.find(cell1),S.find(cell2));
                      
                      vWalls[col][row]=false;
                  }
                  if (S.size(cell1) == vert*horiz)
                      return;

              }
          }else {  // add horiz for odd
              row = random.nextInt(vert-1);
              col = random.nextInt(horiz);
              if(hWalls[col][row]){
                  
                  
                  int cell1=row*horiz+col;
                  int cell2=cell1+horiz;
                  
                  if( S.find(cell1) != S.find(cell2)){
                      S.union(S.find(cell1),S.find(cell2));
                      hWalls[col][row]=false;
                  }
                  if (S.size(cell1) == vert*horiz)
                      return;
