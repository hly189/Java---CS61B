...
public class DList extends List {
   ...
   *  Performance:  runs in O(1) time.
   */
    public void delete(DListNode N){
	N.prev.next = N.next;
	N.next.prev = N.prev;
	size--;
    }
  ...
  public String toString() {
    ...
    while (current != head) {
    	int [] a = {1};
    	if(current.item.getClass() == a.getClass())
	{
		result=result + "["+((int[])current.item)[0];
		for(int n=1;n<((int[])current.item).length;n++)
			result = result + ","+((int[])current.item)[n];
		result = result + "]";
	}
	else
